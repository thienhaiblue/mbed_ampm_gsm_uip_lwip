...
struct CALL_LIST_TYPE incomingCall;
struct CALL_LIST_TYPE *voiceCall;
...
void Ampm_VoiceCallSetAction(struct CALL_LIST_TYPE *call,CALL_ACTION action)
{
...
void Ampm_VoiceCallAdd(struct CALL_LIST_TYPE *call)
{
	list_add(callList, call);
}

void Ampm_VoiceCallCancel(struct CALL_LIST_TYPE *call)
{
...
uint16_t Ampm_VoiceCallSetup(struct CALL_LIST_TYPE *call,
uint8_t *phone,
RINGBUF *dtmf,
CALL_ACTION action,
void (*callback)(void),
...
uint8_t Ampm_CallTask(void)
{
	...
	static struct CALL_LIST_TYPE *call = NULL;
