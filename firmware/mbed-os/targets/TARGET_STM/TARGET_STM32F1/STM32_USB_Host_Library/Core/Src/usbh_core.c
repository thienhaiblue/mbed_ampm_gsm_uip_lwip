...
//static uint32_t thread_stack_usbh[512 / sizeof(uint32_t)];

#if (USBH_USE_OS == 1)  
static void USBH_Process_OS(void const * argument);
osMessageQDef(USBH_Queue, 512, uint16_t);
osThreadDef(USBH_Process_OS, osPriorityNormal, 2048);
#endif

//osThreadDef_t os_thread_def_usbh = {(os_pthread)USBH_Process_OS, osPriorityNormal, 1U, sizeof(thread_stack_usbh), thread_stack_usbh};
/**
  * @brief  HCD_Init 
...
USBH_StatusTypeDef  USBH_Init(USBH_HandleTypeDef *phost, void (*pUsrFunc)(USBH_HandleTypeDef *phost, uint8_t ), uint8_t id)
{
  ...
  phost->os_event = osMessageCreate (osMessageQ(USBH_Queue), NULL); 
	//phost->thread = osThreadCreate (&os_thread_def_usbh, phost);
