...
)
{
    ...
    for(DWORD s=sector; s<sector+count; s++) {
        ...
        int res = FATFileSystem::_ffs[drv]->disk_read((uint8_t*)buff, s,1);
        if(res) {
            return RES_PARERR;
        }
        buff += 512;
...
)
{
    ...
    for(DWORD s = sector; s < sector + count; s++) {
        ...
        int res = FATFileSystem::_ffs[drv]->disk_write((uint8_t*)buff, s,1);
        if(res) {
            return RES_PARERR;
        }
        buff += 512;
    ...
    switch(ctrl) {
        ...
        case GET_SECTOR_COUNT:
            if(FATFileSystem::_ffs[drv] == NULL) {
                ...
                DWORD res = FATFileSystem::_ffs[drv]->disk_sector_count();
                if(res > 0) {
